...
//declare variables
    ...
    const mwSize *dims;
    const mwSize *dims_space;
    double *image, *mask, *c, sigma;
    int MXS, MYS, MZS, numdims, Num_features, Num_points;
    int x,y,z;
  ...
  for(x=0;x<MXS;x++){
      for(y=0;y<MYS;y++)
    {
              ...
              if ((int)(mask[pixelIndex])==10 ){
                  ...
                  gc.setDataCost(pixelIndex,3,10000.0);  
              }
                  ...
                  else if ((int)(mask[pixelIndex])==4){
                  gc.setDataCost(pixelIndex,0,10000.0); 
                  gc.setDataCost(pixelIndex,1,10000.0);
                  gc.setDataCost(pixelIndex,2,10000.0);
                  gc.setDataCost(pixelIndex,3,0.0); 
              } 
