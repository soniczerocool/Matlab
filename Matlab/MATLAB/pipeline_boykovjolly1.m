...
[space,mask_idx,truth_idx] = make_space_test(T1C, FLAIR, T2,truth, MASK);% changed the way the space is created
...
% space_test(2,:) = space(2,:)*max(T2(:));
 ...
 lf = boykov_jolly_3d_multifeature(space_test,MASK,0.005);
 [h,w,d] = size(T1C);
 lf = reshape(lf,h, w, d);
