...
% [width, height, depth] = size(T1C);
 for DEP = 1:depth
    for ROW = 1:height
        for COL=1:width
            if T1C(ROW,COL,DEP) + T2(ROW,COL,DEP) + FLAIR(ROW,COL,DEP) < 0.0001
                continue;
            end
...
for count = 1:length(importance_map)
        ...
        ROW = importance_map(4,count) * height;
        COL = importance_map(5,count) * width;
        DEP = importance_map(6,count) * depth;
