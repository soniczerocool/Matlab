...
//figure out dimensions
    ...
    MYS = (int)dims[0]; MXS = (int)dims[1];  MZS = (int)dims[2];
...
for(z=0;z<MZS;z++) {       
    for(y=0;y<MYS;y++)
    {
          for(x=0;x<MXS;x++)
          {
            int pixelIndex =z*(MXS*MYS)+ y*MXS+x;
    ...
    for(z=0;z<MZS;z++){
        for (int y = 0; y < MYS; y++ ){
                for (int  x = 1; x < MXS; x++ ){
                        int pixelIndex1 = z*MXS*MYS+y*MXS+x;
                        int pixelIndex2 = z*MXS*MYS+y*MXS+x-1;
                         gc.setNeighbors(z*MXS*MYS+x+y*MXS,z*MXS*MYS+x-1+y*MXS); 
        ...
        for(z=0;z<MZS;z++){ 
            for (int y = 1; y < MYS; y++ ){
                    for (int  x = 0; x < MXS; x++){
                            ...
                            int pixelIndex1 = z*MXS*MYS+y*MXS+x;
                            int pixelIndex2 = z*MXS*MYS+(y-1)*MXS+x;
                            gc.setNeighbors(z*MXS*MYS+x+y*MXS,z*MXS*MYS+x+(y-1)*MXS);
                    }
        }
        }
        for(z=1;z<MZS;z++){ 
            for (int y = 0; y < MYS; y++ ){
                    for (int  x = 0; x < MXS; x++){
                            ...
                            int pixelIndex1 = z*MXS*MYS+y*MXS+x;
                            int pixelIndex2 = (z-1)*MXS*MYS+y*MXS+x;
                            gc.setNeighbors(z*MXS*MYS+x+y*MXS,(z-1)*MXS*MYS+x+y*MXS);
                    }
        }
        }
